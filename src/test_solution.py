...
@pytest.mark.it("The dictionary returned by GET /members/<int:id> should contain one family member with the keys [name, id, age, lucky_numbers]")
...
@pytest.mark.it("Method GET /members/1 should return Tommy")
...
def test_delete_member(client):
    response = client.delete('/members/4')
    assert response.status_code == 200

@pytest.mark.it("Method DELETE /members/6 should delete created family member")
def test_delete_response(client):
    ...
    response = client.delete('/members/6')
    assert response.status_code == 200

@pytest.mark.it("After deleting the member 6 we called GET /members and it should return a list with 4 members")
